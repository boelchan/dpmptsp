    ...
    public function dataTable($query)
    {
        ...
        return $dataTable->editColumn('created_at', function ($query) {
            ...
            ->editColumn('role', function ($query) {
                $role = Str::upper($query->roles->first()->name);
                if ($query->hasRole('instansi')) {
                    $role .= '<br>'.$query->instansi->nama;
                }
            ...
            ->editColumn('action', function ($query) {
                return view('components.button.show', ['action' => route('user.show', [$query->id, 'uuid' => $query->uuid])]).
                    ...
                    ($query->id == 1 ? '' : view('components.button.destroy', ['action' => route('user.destroy', [$query->id, 'uuid' => $query->uuid]), 'label' => $query->name, 'target' => 'user-table']));
            })
            ->rawColumns(['role', 'banned_at', 'action']);
    }
